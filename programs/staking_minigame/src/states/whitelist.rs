...
pub struct Whitelist {
    ...
    // Whitelist type, 0 for token reference, 1 for creator reference
    pub whitelist_type: u8,
    
    // Faction, 0 for none, 1 for good, 2 for bad
    pub faction: u8,
...
impl Whitelist {
    ...
    fn assert_metadata(&self, mint: &Pubkey, mint_metadata: &AccountInfo) -> Result<()> {
        ...
        require_keys_eq!(
            *mint_metadata.owner,
            metadata_program,
            StakingErrors::InvalidMetadataAccount
        );
    ...
    fn assert_whitelist(&self, seed_account: &Pubkey, program_account: &Pubkey, whitelist: &Pubkey) -> Result<()> {
        ...
        require_keys_eq!(
            whitelist_account,
            *whitelist,
            StakingErrors::InvalidWhitelistProof
        );
    ...
    pub fn verify(&self, program_account: &Pubkey, whitelist: &Pubkey, mint: &Pubkey, mint_metadata: &AccountInfo) -> Result<()> {
        ...
        Err(error!(StakingErrors::InvalidWhitelistProof))
    }
